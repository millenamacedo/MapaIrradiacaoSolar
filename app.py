...
Faça o upload de um arquivo **CSV** com as colunas:  
`LON`, `LAT`, `ANNUAL` — usando **; (ponto e vírgula)** como separador e **, (vírgula)** como decimal.
""")
...
if uploaded_file is not None:
    try:
        ...
        if all(col in df.columns for col in ['LON', 'LAT', 'ANNUAL']):
            # Criação do mapa centralizado no Brasil
            m = folium.Map(location=[-14.235, -51.9253], zoom_start=4)

            # Define cores conforme a faixa de irradiação
